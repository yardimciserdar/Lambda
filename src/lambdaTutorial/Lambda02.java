...
public class Lambda02 {
    ...
    // Task : Functional Programming ile listin cift elemanlarinin  karelerini ayni satirda aralarina
    ...
    // Task : Functional Programming ile listin cift elemanlarinin   karekoklerini ayni
    // satirda aralarina bosluk birakarak yazdiriniz
    public static void ciftKarekokPrint(List<Integer> sayi){
        sayi.
                stream().
                filter(Lambda01::ciftBul).
                map(Math::sqrt).//Math ref--> double deger return eder
                //forEach(lambdaTutorial.Lambda01::yazdir)--> yazdir() method int.  çalıştığı için
                // map den çıkan dataları çalıştırmaz
    ...
    public static void maxElBul(List<Integer> sayi){
       Optional<Integer> maxsayi =
        ...
        System.out.println("serdarca " + sayi.stream().reduce(Math::max));
        /*
        reduce()-->azaltmak ... bir cok datayi tek bir dataya(max min carp top vs islemlerde) cevirmek icin kullanilir.
        kullanımı yaygındır pratiktir.
        Bir Stream içerisindeki verilerin teker teker işlenmesidir. Teker teker işleme sürecinde, bir önceki adımda
        elde edilen sonuç
    ...
    public static void ciftKarteMaxBul(List<Integer> sayi) {
        Optional<Integer> maxciftkare = sayi.stream().filter(Lambda01::ciftBul).map(t -> t * t).reduce(Math::max);
        System.out.println(maxciftkare);//36

        System.out.println("daha hızlı specific Integer class çalıştı " + sayi.
                stream().
                filter(Lambda01::ciftBul).
                map(t -> t * t).
    ...
    public static void minBul(List<Integer> sayi){
       ...
       Optional<Integer> minSayiMath = sayi.stream().reduce(Math::min);
        ...
        Optional<Integer> minSayiSerdar = sayi.stream().reduce(Lambda02::bySerdarMin);
        System.out.println(minSayiSerdar);

    }

    public static int bySerdarMin(int a , int b){//bu method kendisine verilen iki int degerin en kücügunu return eder
         return a < b ? a : b ;

    }
    // Task : List'teki 5'ten buyuk en kucuk tek sayiyi print ediniz.
    public static void bestenBykEnKck(List<Integer> sayi){
        System.out.println(sayi.stream().filter(t-> t > 5 && t % 2 ==1 ).reduce(Lambda02::bySerdarMin));
